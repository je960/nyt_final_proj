...
test_hist_3 <- test_history %>%
  mutate(perc_pos = positive/total_test_results) %>%
...
omg <- test_hist_3 %>% 
  ...
  mutate(people_per_test = (total_test_results/pop), 
         state = as.factor(state)) %>% 
  select(state, perc_pos, people_per_test)

omg %>%
  ggplot(aes(x = people_per_test, y = perc_pos)) + 
  geom_point() +
  theme_minimal()
```
